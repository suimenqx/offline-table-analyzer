...
- 增量加固页签重命名与拖拽排序，不改动解析、配置规则和新建页签流程。
- 新增 `activateTab()`，点击当前激活页签不再重复渲染，避免双击重命名被第一次 click 触发的重绘打断。
- 双击非激活页签时会先激活再进入原地重命名，仍支持 Enter 保存、Esc 取消、失焦保存。
- 拖拽排序继续保持 active tab 不变，并补充拖到页签栏空白区域时放到末尾的处理。
- UI 静态测试增加对双击重命名和拖拽绑定的校验。
