...
function buildSingleTableResult(rows, name, sourceType, options={}, meta={}, forcedHeader) {
    ...
    return {
        tables:[{ name, headers:resolved.headers, rows:resolved.rows, sourceType, meta:{ ...meta, hasHeader:resolved.hasHeader, generatedHeaders:resolved.generatedHeaders }, diagnostics:resolved.diagnostics }],
...
const SemicolonCsvParser = createDelimitedParser({
    ...
    confidence(source) {
        ...
        return st.consistent ? st.score - 0.04 : 0;
    }
});
