...
const TableUtils = {
    ...
    trimRow(row=[]) { return row.map(v => String(v ?? '').replace(/\u00a0/g, ' ').trim()); },
    ...
    normalizeRows(rows=[], width=0, diagnostics=[], tableName='Table') {
        ...
        rows.forEach((row, idx) => {
            ...
            if(width && r.length > width) r.length = width;
            out.push(r);
        });
        return out;
    },
        ...
        const parseTableHtml = (tableHtml, idx) => {
            ...
            trMatches.forEach(trHtml => {
                ...
                while((m = cellRe.exec(trHtml)) !== null) {
                    ...
                    for(let i=0; i<span; i++) cells.push(i === 0 ? value : '');
                }
...
const CsvParser = {
    id:'csv', label:'CSV', delimiter:',',
    confidence(source) {
        const text = source.text || '';
        if(/\t/.test(text)) return 0.05;
        const st = Delimited.delimiterStats(text, ',');
        return st.consistent ? st.score : 0.12;
    },
    parse(source, options={}) { return parseDelimitedTable(source.text, ',', this.id, options); }
};

const SemicolonCsvParser = {
    id:'semicolon-csv', label:'分号分隔', delimiter:';',
    confidence(source) { const st = Delimited.delimiterStats(source.text || '', ';'); return st.consistent ? st.score - 0.04 : 0; },
    parse(source, options={}) { return parseDelimitedTable(source.text, ';', this.id, options); }
};

const ExcelPasteParser = {
    id:'excel-paste', label:'Excel/表格复制 TSV', delimiter:'\t',
    confidence(source) {
        const text = source.text || '';
...
const PipeTableParser = {
    ...
    parse(source, options={}) {
        ...
        const name = 'Pipe Table 1';
        const resolved = HeaderResolver.infer(rows, { ...options, hasHeader: hasMdSep || options.hasHeader, tableName:name });
        return { tables:[{ name, headers:resolved.headers, rows:resolved.rows, sourceType:this.id, meta:{ hasHeader:resolved.hasHeader, generatedHeaders:resolved.generatedHeaders }, diagnostics:resolved.diagnostics }], diagnostics:resolved.diagnostics };
    }
};
...
const AsciiTableParser = {
    ...
    parse(source, options={}) {
        ...
        const name = 'ASCII Table 1';
        const resolved = HeaderResolver.infer(rows, { ...options, tableName:name });
        return { tables:[{ name, headers:resolved.headers, rows:resolved.rows, sourceType:this.id, meta:{ hasHeader:resolved.hasHeader, generatedHeaders:resolved.generatedHeaders }, diagnostics:resolved.diagnostics }], diagnostics:resolved.diagnostics };
    }
};
...
const FixedWidthParser = {
    ...
    parse(source, options={}) {
        ...
        const name = 'Fixed Width Table 1';
        const resolved = HeaderResolver.infer(rows, { ...options, tableName:name });
        return { tables:[{ name, headers:resolved.headers, rows:resolved.rows, sourceType:this.id, meta:{ hasHeader:resolved.hasHeader, generatedHeaders:resolved.generatedHeaders }, diagnostics:resolved.diagnostics }], diagnostics:resolved.diagnostics };
    }
};
...
const CliTableDataParser = {
    ...
    confidence(source) { return /(^|\n)\s*table-data\b/i.test(source.text || '') ? 1 : 0; },
    parse(source, options={}) {
        const lines = TableUtils.lines(source.text);
        const tables = [];
        const diagnostics = [];
        ...
        const finalize = () => {
            ...
            if(cur.headers.length) {
                const width = cur.headers.length;
                cur.rows = TableUtils.normalizeRows(cur.rows, width, diagnostics, cur.name);
                cur.sourceType = this.id;
                cur.meta = { mode:cur.mode || 'WS', hasHeader:true, generatedHeaders:false };
                cur.diagnostics = diagnostics.filter(d => d.table === cur.name);
                tables.push(cur);
            }
            cur = null; ranges = [];
        };
        const parseByMode = (line) => {
            if(cur.mode === 'TAB') return Delimited.parse(line, '\t')[0] || [];
            if(cur.mode === 'CSV') return Delimited.parse(line, ',')[0] || [];
            if(cur.mode === 'PIPE') return line.trim().replace(/^\||\|$/g, '').split('|').map(v => v.trim());
        ...
        const setHeader = (line) => {
            if(line.includes('\t')) { cur.mode = 'TAB'; cur.headers = TableUtils.ensureUniqueHeaders(Delimited.parse(line.trim(), '\t')[0] || []); return; }
            if(line.includes(',') && !/\s{2,}/.test(line)) { cur.mode = 'CSV'; cur.headers = TableUtils.ensureUniqueHeaders(Delimited.parse(line.trim(), ',')[0] || []); return; }
            if((line.match(/\|/g) || []).length >= 2) { cur.mode = 'PIPE'; cur.headers = TableUtils.ensureUniqueHeaders(line.trim().replace(/^\||\|$/g, '').split('|').map(v=>v.trim())); return; }
            ranges = [];
            ...
            cur.mode = /\s{2,}/.test(line) ? 'FIXED' : 'WS';
            cur.headers = TableUtils.ensureUniqueHeaders(cur.mode === 'FIXED' ? ranges.map(r => line.substring(r.s, Math.min(r.e, line.length)).trim()) : line.trim().split(/\s+/));
        };
        lines.forEach((line) => {
            const trim = line.trim();
            const mt = /^\s*table-data\s+([^\s|]+)/i.exec(line);
            if(mt) {
                finalize();
                const name = TableUtils.makeTableName(mt[1], tables.length, used);
                cur = { name, headers:[], rows:[], mode:'WS' };
                return;
            }
            if(!cur || !trim) return;
            if(!cur.headers.length) { setHeader(line); return; }
...
function parseDelimitedTable(text, delimiter, sourceType, options={}) {
    const matrix = Delimited.parse(text || '', delimiter);
    const nameMap = { csv:'CSV Table 1', 'semicolon-csv':'Delimited Table 1', 'excel-paste':'Excel Paste Table 1' };
    const name = nameMap[sourceType] || 'Delimited Table 1';
    const resolved = HeaderResolver.infer(matrix, { ...options, tableName:name });
    return { tables:[{ name, headers:resolved.headers, rows:resolved.rows, sourceType, meta:{ delimiter, hasHeader:resolved.hasHeader, generatedHeaders:resolved.generatedHeaders }, diagnostics:resolved.diagnostics }], diagnostics:resolved.diagnostics };
}

const ImportEngine = {
    parsers: [CliTableDataParser, HtmlTableParser, AsciiTableParser, PipeTableParser, ExcelPasteParser, CsvParser, SemicolonCsvParser, FixedWidthParser, PlainTextTableParser],
    parse(input, options={}) {
        ...
        const candidates = this.parsers.map(parser => ({ parser, type:parser.id, label:parser.label, confidence: Math.max(0, Math.min(1, parser.confidence(source, options) || 0)) }))
            .filter(c => c.confidence > 0)
            .sort((a,b) => b.confidence - a.confidence);
        const selectedType = options.format && options.format !== 'auto' ? options.format : null;
        const chosen = selectedType ? candidates.find(c => c.type === selectedType) || this.parsers.find(p => p.id === selectedType) && {parser:this.parsers.find(p => p.id === selectedType), type:selectedType, label:this.parsers.find(p => p.id === selectedType).label, confidence:1} : candidates[0];
        if(!chosen) return { tables:[], format:'empty', label:'空输入', confidence:0, candidates:[], diagnostics:[] };
        const parsed = chosen.parser.parse(source, options);
        const diagnostics = parsed.diagnostics || [];
        if(!parsed.tables.length) diagnostics.push({ level:'warning', code:'NO_TABLES', message:'没有识别到可用表格数据' });
        return { tables:parsed.tables, format:chosen.type, label:chosen.label, confidence:chosen.confidence, candidates:candidates.map(c => ({type:c.type, label:c.label, confidence:c.confidence})), diagnostics };
    }
};

/* Parser */
const Parser = {
    lastResult: { tables:[], format:'empty', label:'空输入', diagnostics:[], candidates:[] },
    parse(txt, options={}) {
        try {
            ...
            this.lastResult = { tables:[], format:'error', label:'解析错误', diagnostics:[{level:'error', code:'PARSE_ERROR', message:e.message}], candidates:[] };
            return [];
        }
    }
};
...
/* Parser regression tests, exposed for development and safe to ignore in production. */
window.__OTA_TESTS__ = {
    assert(cond, msg) { if(!cond) throw new Error(msg); },
    runAll() {
        const cases = [];
        const test = (name, fn) => cases.push({name, fn});
        test('cli-table-data parses legacy multi-table data', () => {
            const r = ImportEngine.parse('table-data Users\nvalidflag ID Name\n1 100 Alice\n1 101 Bob\n\ntable-data Orders\nvalidflag OrderID UserID\n1 5001 100');
            this.assert(r.tables.length === 2, 'expected 2 tables');
            this.assert(r.tables[0].headers.includes('validflag'), 'expected validflag header');
            this.assert(r.tables[0].rows.length === 2, 'expected rows');
        });
        test('csv parser supports quoted comma and escaped quotes', () => {
            const r = ImportEngine.parse('id,name,comment\n1,Alice,"hello, world"\n2,Bob,"a ""quoted"" value"');
            this.assert(r.format === 'csv', 'expected csv');
            this.assert(r.tables[0].rows[0][2] === 'hello, world', 'quoted comma failed');
            this.assert(r.tables[0].rows[1][2] === 'a "quoted" value', 'escaped quotes failed');
        });
        test('header resolver generates friendly headers', () => {
            const r = ImportEngine.parse('1,Alice,20\n2,Bob,30');
            this.assert(r.tables[0].headers[0] === 'Column1', 'expected generated Column1');
            this.assert(r.tables[0].rows.length === 2, 'first row must remain data');
        });
        test('excel copied data parses as tab-delimited', () => {
            const r = ImportEngine.parse('id\tname\tage\n1\tAlice\t20\n2\tBob\t30');
            this.assert(r.format === 'excel-paste', 'expected excel-paste');
            this.assert(r.tables[0].headers[1] === 'name', 'expected header');
        });
        test('markdown pipe table parses header separator', () => {
            const r = ImportEngine.parse('| id | name |\n|----|------|\n| 1 | Alice |');
            this.assert(r.format === 'pipe-table', 'expected pipe table');
            this.assert(r.tables[0].headers[0] === 'id', 'expected header');
        });
        test('ascii table parses terminal borders', () => {
            const r = ImportEngine.parse('+----+-------+\n| id | name  |\n+----+-------+\n| 1  | Alice |\n+----+-------+');
            this.assert(r.format === 'ascii-table', 'expected ascii table');
            this.assert(r.tables[0].rows[0][1] === 'Alice', 'expected Alice');
        });
        test('html table parser uses clipboard html', () => {
            const html = '<table><tr><th>id</th><th>name</th></tr><tr><td>1</td><td>Alice</td></tr></table>';
            const r = ImportEngine.parse({ text:'id\tname\n1\tAlice', html });
            this.assert(r.format === 'html-table', 'expected html table');
            this.assert(r.tables[0].headers[1] === 'name', 'expected html header');
        });
        const failures = [];
        cases.forEach(c => { try { c.fn(); } catch(e) { failures.push(`${c.name}: ${e.message}`); } });
...
const App = {
    ...
    updateImportSummary() {
        ...
        if(!res.format || res.format === 'empty') { el.classList.remove('show'); el.innerHTML = ''; return; }
        const tableCount = (res.tables || []).length;
        const rowCount = (res.tables || []).reduce((sum, t) => sum + (t.rows || []).length, 0);
        ...
        const html = [
            ...
            `<span class="summary-pill">置信度: ${pct}%</span>`,
    ...
    showImportDiagnostics() {
        const list = (Parser.lastResult && Parser.lastResult.diagnostics) || [];
        if(!list.length) return Toast.show('没有解析警告');
        const html = `<div style="display:flex; flex-direction:column; gap:8px;">${list.map(d => `<div style="padding:10px; border:1px solid var(--border); border-radius:8px; background:var(--bg-elevated);"><div class="bold">${d.code || d.level}</div><div>${d.message || ''}</div></div>`).join('')}</div>`;
...
validflag Time      Level   Message                 Code
 ...
 1        10:15:01  INFO    Retry connection...     0x00`;
        ...
        $('rawInput').addEventListener('paste', e => {
            ...
            if(html && /<table[\s>]/i.test(html)) this.lastPaste = { html, plain };
        });
    ...
    run(render=true) {
        try {
            ...
            this.raw = Parser.parse($('rawInput').value, this.getParseOptions());
